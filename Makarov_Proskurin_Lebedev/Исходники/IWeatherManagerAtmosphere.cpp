...
IWeatherManagerAtmosphere::IWeatherManagerAtmosphere(f32 dayspeed, f32 phi, f32 theta, f32 rotation,
	...
	DaySpeed(dayspeed), Phi(phi), Theta(theta), Rotation(rotation), FirstUpdate(true), LastAnimationTime(0)
{
	if(Phi < 0.f)
		Phi = -Phi;
	if(Theta < 0.f)
	...
	// Start the sky
	setSkyImage(driver->createImageFromFile("Textures/skycolor.png"));
	CreateSkyPalette();
...
void IWeatherManagerAtmosphere::CreateSkyPalette()
{
	...
	scene::ISceneNode* skybox = smgr->addSkyBoxSceneNode(
		driver->getTexture("Textures/stars_top.png"),
		driver->getTexture("Textures/stars_bottom.png"),
		driver->getTexture("Textures/stars.png"),
		driver->getTexture("Textures/stars.png"),
		driver->getTexture("Textures/stars.png"),
		driver->getTexture("Textures/stars.png"));
	...
	Sky = new IAtmosphereSkySceneNode(smgr->getRootSceneNode(), smgr, -1, driver->getTexture("Textures/sky.png"), 16, Radius * 10.f);

	// Sun billboard
	video::ITexture* sun = driver->getTexture("Textures/sun.tga");
	video::ITexture* moon = driver->getTexture("Textures/moon.tga");
	Bill = new IAtmosphereStarSceneNode(smgr->getRootSceneNode(), smgr, -1, sun, moon, core::dimension2df(128.f, 128.f));

//    Sky->setDebugDataVisible(scene::EDS_FULL);
//    Bill->setDebugDataVisible(scene::EDS_FULL);

//    Sunlight = smgr->addLightSceneNode(Bill, core::vector3df(0.f, 0.f, 0.f),
...
void IWeatherManagerAtmosphere::render()
{
	...
	Bill->setSunPosition(billboardPosition);

	billboardPosition = -sun_place;
	Bill->setMoonPosition(billboardPosition);
